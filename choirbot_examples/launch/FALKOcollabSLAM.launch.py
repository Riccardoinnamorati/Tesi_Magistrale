...
keypoints_detection_freq = 2        # [Hz]
freq_keypoint_matching = 0.5          # [Hz]
FALKO = True  # Set to True to use FALKO, False for simple guidance
...
def generate_launch_description():
    ...
    source_filename = os.path.join(world_package_dir, 'worlds', 'empty_mapping_world.wbt' if not FALKO else 'empty_FALKO_world.wbt')
    ...
    for i in range(N):
        ...
        launch_description.append(Node(
            package='choirbot_examples',
            executable='choirbot_FALKOcollabSLAM_simple_guidance', 
            output='screen',
        ...
        launch_description.append(Node(
            package='choirbot_examples',
            executable='choirbot_FALKOcollabSLAM_guidance', 
            output='screen',
            prefix=f'xterm  -title "guidance_{i}" -geometry 160x7+1980+{int(135*(i))} -hold -e ', # if i == 0 else None,
            namespace=f'agent_{i}',
            parameters=[{
                ...
                'keypoints_detection_freq': keypoints_detection_freq,
                'agent_id': i, 
                'N': N, 
                'in_neigh': in_neighbors, 
                'out_neigh': out_neighbors, 
        ...
        # controller
        launch_description.append(Node(
            package='choirbot_examples', executable='choirbot_FALKOcollabSLAM_controller', output='screen',
            namespace=f'agent_{i}',
